...
// copy paste from: https://github.com/ephtracy/voxel-model/blob/master/MagicaVoxel-file-format-vox.txt
static uint32_t s_defaultPalette[256] = {
...
{
	r = (uint8_t)(rgba & 0x000000ff) >> 0;
	g = (uint8_t)(rgba & 0x0000ff00) >> 8;
	b = (uint8_t)(rgba & 0x00ff0000) >> 16;
...
Palette::Palette()
{
	...
	{
		m_colors[i].fromRgba(s_defaultPalette[i]);
	}
}

error Palette::load(Loader& loader)
{
	...
	for(size_t i=0; i<NumColors; i++)
	{
		...
		color.r = (uint8_t)loader.readNextChar();
		color.g = (uint8_t)loader.readNextChar();
